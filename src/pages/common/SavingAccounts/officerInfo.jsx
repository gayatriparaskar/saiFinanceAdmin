...
<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
  ...
  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-center">
    <div className="text-lg sm:text-xl font-bold text-blue-600">
      {t('Total Collections')} : {userCollections.length}
    </div>
  </div>

  <div className="bg-green-50 border border-green-200 rounded-lg p-4 text-center">
    <div className="text-lg sm:text-xl font-bold text-green-600">
      {t('Total Amount')} : ₹ {userCollections.reduce((sum, c) => sum + (c.collected_amount || c.amount || 0), 0).toLocaleString()}
    </div>
  </div>
</div>
    ...
    <span className="text-xs sm:text-sm text-gray-500">
      {reportType === 'daily' ? t('Today') :
       ...
       t('This Month')}
        ...
        {/* Edit Officer Drawer */}
        <Drawer isOpen={isEditing} placement="right" onClose={() => setIsEditing(false)} size={["full", "md"]}>
          <DrawerOverlay />
          <DrawerContent>
            <DrawerCloseButton />
