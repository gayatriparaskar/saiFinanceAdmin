...
function SavingAccount() {
  ...
  const calculateFutureAmount = (currentAmount, interestRate = 5, days = 120) => {
    ...
    // Convert days to years (120 days = 120/365 years)
    const years = days / 365;
    
    // Calculate compound interest: A = P(1 + r/100)^t
    const futureAmount = currentAmount * Math.pow(1 + (interestRate / 100), years);
  ...
  const columns = React.useMemo(
    () => [
      ...
      },
      {
        ...
        Cell: ({ value, row: { original } }) => {
          const totalAmount = original?.saving_account_id?.total_amount || 0;
  ...
  return (
    <>
      ...
      {/* Interest Calculation Detail Modal */}
      <AlertDialog isOpen={isDetailOpen} onClose={onDetailClose} size="xl">
        <AlertDialogOverlay>
          <AlertDialogContent maxW="600px">
            <AlertDialogHeader fontSize="lg" fontWeight="bold">
              <div className="flex items-center">
                <span className="text-green-500 mr-2">💰</span>
                {t('Total Amount Details')}
              </div>
            </AlertDialogHeader>
            <AlertDialogBody>
              {selectedAccount && (
                <div className="space-y-4">
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <h3 className="font-semibold text-gray-800 mb-2">{t('Account Information')}</h3>
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        <span className="text-gray-600">{t('Account Holder')}:</span>
                        <span className="ml-2 font-medium">{selectedAccount.full_name}</span>
                      </div>
                      <div>
                        <span className="text-gray-600">{t('Account Number')}:</span>
                        <span className="ml-2 font-medium">{selectedAccount.saving_account_id?.account_number || 'N/A'}</span>
                      </div>
                      <div>
                        <span className="text-gray-600">{t('Phone')}:</span>
                        <span className="ml-2 font-medium">{selectedAccount.phone_number}</span>
                      </div>
                      <div>
                        <span className="text-gray-600">{t('Officer')}:</span>
                        <span className="ml-2 font-medium">{selectedAccount.officer_id?.name || 'N/A'}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-blue-50 p-4 rounded-lg">
                    <h3 className="font-semibold text-blue-800 mb-2">{t('Current Investment')}</h3>
                    <div className="text-2xl font-bold text-blue-600">
                      ₹ {selectedAccount.saving_account_id?.current_amount?.toLocaleString() || 0}
                    </div>
                  </div>

                  {/* Total Amount Information */}
                  {/* <div className="bg-green-50 p-4 rounded-lg">
                    <h3 className="font-semibold text-green-800 mb-3">{t('Total Amount Breakdown')}</h3>
                    
                    {(() => {
                      const currentAmount = selectedAccount.saving_account_id?.current_amount || 0;
                      const totalAmount = selectedAccount.saving_account_id?.total_amount || 0;
                      const emiAmount = selectedAccount.saving_account_id?.emi_amount || 0;
                      const emiDays = selectedAccount.saving_account_id?.emi_day || 120;
                      
                      return (
                        <div className="space-y-3">
                          <div className="flex justify-between items-center">
                            <span className="text-gray-600">{t('Daily EMI Amount')}:</span>
                            <span className="font-medium">₹ {emiAmount.toLocaleString()}</span>
                          </div>
                          
                          <div className="flex justify-between items-center">
                            <span className="text-gray-600">{t('Total EMI Days')}:</span>
                            <span className="font-medium">{emiDays} days</span>
                          </div>
                          
                          <div className="flex justify-between items-center">
                            <span className="text-gray-600">{t('Current Amount')}:</span>
                          ...
                          <div className="flex justify-between items-center text-lg">
                            <span className="font-semibold text-gray-800">{t('Total Amount')}:</span>
                            <span className="font-bold text-green-600 text-xl">₹ {totalAmount.toLocaleString()}</span>
                          </div>
                          
                          <div className="text-sm text-gray-500 mt-2">
                            {t('Calculation')}: Daily EMI × Total Days = ₹{emiAmount.toLocaleString()} × {emiDays} = ₹{totalAmount.toLocaleString()}
                          </div>
                        </div>
                      );
                    })()}
                  ...
                  {/* <div className="bg-yellow-50 p-4 rounded-lg">
                    <h3 className="font-semibold text-yellow-800 mb-2">{t('Important Notes')}</h3>
                    <ul className="text-sm text-yellow-700 space-y-1">
                      <li>• {t('Interest is calculated using compound interest formula')}</li>
                      <li>• {t('Rate: 5% per annum for 120 days')}</li>
                      <li>• {t('Amount shown is the projected value after 120 days')}</li>
                      <li>• {t('Actual returns may vary based on account terms')}</li>
                    </ul>
                  </div> */}
                </div>
              )}
            ...
            <AlertDialogFooter>
              <Button onClick={onDetailClose} colorScheme="blue">
                {t('Close')}
              </Button>
            </AlertDialogFooter>
          </AlertDialogContent>
