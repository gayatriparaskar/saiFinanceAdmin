...
const ManagerOfficer = () => {
  ...
  const currentUser = JSON.parse(localStorage.getItem("officerType"));
  console.log(currentUser,"currentUser");
  
  const isAccounter = currentUser === "accounter";
  ...
  const fetchData = async () => {
    try {
      ...
      if (response?.data?.result) {
        ...
        setActiveOfficers(
          officersData.filter((officer) => officer.is_active).length
        );
      }
    } catch (error) {
  ...
  const handleSort = (sortKey) => {
    ...
    switch (sortKey) {
      ...
      case "status_inactive":
        sortedData.sort(
          (a, b) => (a.is_active ? 1 : 0) - (b.is_active ? 1 : 0)
        );
        break;
      case "date_newest":
        sortedData.sort(
      ...
      case "date_oldest":
        sortedData.sort(
          (a, b) => new Date(a.created_on || 0) - new Date(b.created_on || 0)
        );
        break;
      default:
        break;
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        Cell: ({ value, row: { original } }) => (
          <div className="flex items-center">
            ...
            <div className="ml-3">
              <Cell
                text={`${original?.name || original?.full_name}`}
                bold={"bold"}
              />
      ...
      },
      {
        ...
        Cell: ({ value, row: { original } }) => (
          <span
            className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getOfficerTypeColor(
              original?.role
            )}`}
          >
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
              <div className="search-section flex flex-col sm:flex-row gap-4 flex-1">
                ...
                <Menu>
                  ...
                  <MenuList>
                    <MenuItem onClick={() => handleSort("name_a_to_z")}>
                      {t("Name (A to Z)")}
                    </MenuItem>
                    <MenuItem onClick={() => handleSort("name_z_to_a")}>
                      {t("Name (Z to A)")}
                    </MenuItem>
                    <MenuItem onClick={() => handleSort("type_a_to_z")}>
                      {t("Type (A to Z)")}
                    </MenuItem>
                    <MenuItem onClick={() => handleSort("type_z_to_a")}>
                      {t("Type (Z to A)")}
                    </MenuItem>
                    <MenuItem onClick={() => handleSort("status_active")}>
                      {t("Status (Active)")}
                    </MenuItem>
                    <MenuItem onClick={() => handleSort("status_inactive")}>
                      {t("Status (Inactive)")}
                    </MenuItem>
                    <MenuItem onClick={() => handleSort("date_newest")}>
                      {t("Date (Newest)")}
                    </MenuItem>
                    <MenuItem onClick={() => handleSort("date_oldest")}>
                      {t("Date (Oldest)")}
                    </MenuItem>
                  </MenuList>
                </Menu>
              ...
              {editData && (
                <div className="space-y-4">
                  ...
                  <FormControl>
                    ...
                    >
                      <option value="collection_officer">
                        {t("Collection Officer")}
                      </option>
